...
const ForHome = ({weekday, month}) => {
    ...
    useEffect( () => {
        function getData() {
            dataBase.collection("ForHome").orderBy('timestamp', 'desc').onSnapshot(item => 
    ...
    const NewNote = (e) => {
        ...
        dataBase.collection("ForHome").doc(`${Math.random() * 526}`).set({
            id: newNote.id,
                        ...
                        <div className="notes" key={id}>
                            ...
                            >
                                <FormControlLabel
                                    control={<Checkbox 
                                        ...
                                        onChange={() =>                                             
                                            function(){
                                                console.log(weekday[new Date(data.timestamp.toDate()).getDay()])
                                                dataBase.collection("ForHome").doc(id).set({
                                                    id: data.id,
                                                    solved: noteSolved,
                                                    note: data.note,
                            ...
                            <div className={`prompt-user ${noteId === data.id && deleteNote ? "show-prompt" : ""}`}>
                                ...
                                <span 
                                    ...
                                    dataBase.collection("ForHome").doc(id).delete();
                                    setDeleteNote(false);
                                        }()} 
                                >Yes
                                </span>
