...
const Work = ({weekday, month}) => {
    ...
    useEffect( () => {
        function getData() {
            dataBase.collection("Work").orderBy('timestamp', 'desc').onSnapshot(item => 
    ...
    const NewNote = (e) => {
        ...
        dataBase.collection("Work").doc(`${Math.random() * 526}`).set({
            id: newNote.id,
                            ...
                            >
                                <FormControlLabel
                                    control={<Checkbox 
                                        ...
                                        onChange={() =>                                             
                                            function(){
                                                dataBase.collection("Work").doc(id).set({
                                                    id: data.id,
                                                    solved: noteSolved,
                                                    note: data.note,
                            ...
                            <div className={`prompt-user ${ noteId === data.id && deleteNote ? "show-prompt" : ""}`}>
                                ...
                                <span 
                                    ...
                                    onClick={() => function deleteNote(){
                                                dataBase.collection("Work").doc(id).delete();
                                                setDeleteNote(false);
                                        }()} 
                                >
                                    Yes
